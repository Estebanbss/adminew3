...
export class AtractivosService {
  ...
  //Create - C
  agregarAtractivo(atractivo: any, files: any, portadaFile: any): Promise<any> {
    ...
    if (!(files.length === 0)) {
      ...
      for (let file of files) {
        ...
        const imgRef = ref(
          this.storage,
          `atractivosStorage/${atractivo.name}/${file.name}`
        );
      ...
      Promise.all(arregloDePromesas)
        .then((resultados) => {
          ...
          for (let resultado of resultados) {
            ...
            //atractivo.pathImages.push(fullPath); //Guardamos los Paths en nuestro arreglo pathImages
  ...
  obtenerAtractivos(): Observable<AtractivoTuristico[]> {
    ...
    const q = query(atractivoRef, orderBy('name', 'asc'));
  ...
  borrarImagenesAtractivo(atractivo: any) {
    ...
    if (!(pathImgPrincipal === '')) {
      ...
      deleteObject(refImgPrincipal)
        .then(() => {
          console.log('Se ha borrado la img Principal de: ', atractivo.name);
        })
        .catch((error) => {
