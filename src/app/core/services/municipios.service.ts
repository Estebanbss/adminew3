...
export class MunicipiosService {
  ...
  private sharingDataMunicipio: BehaviorSubject<Municipio> =
    new BehaviorSubject<Municipio>({
  ...
  get sharingMunicipio() {
    return this.sharingDataMunicipio.asObservable();
  }

  //? Cambiamos la Información al observable
  set editMunicipioData(newValue: Municipio) {
    this.sharingDataMunicipio.next(newValue);
  }
  ...
  //Create - C
  agregarMunicipio(municipio: any, files: any, portadaFile: any): Promise<any> {
    ...
    if (!(files.length === 0)) {
      ...
      for (let file of files) {
        ...
        const imgRef = ref(
          this.storage,
          `municipiosStorage/${municipio.name}/${file.name}`
        );
      ...
      Promise.all(arregloDePromesas)
        .then((resultados) => {
          ...
          for (let resultado of resultados) {
            ...
            //municipio.pathImages.push(fullPath); //Guardamos los Paths en nuestro arreglo pathImages
  ...
  borrarMunicipio(municipio: any): Promise<any> {
    //Creamos la referencia al documento que queremos borrar
    const docRef = doc(this.firestore, `municipios/${municipio.id}`); // Borramos por id
    return deleteDoc(docRef); // Nos retorna una promesa
  } //? Fin método eleminar municipio para la opción de borrado en el Listado

  //? Aquí borramos los datos de Storage para la opción de borrado en el Listado
  borrarImagenesMunicipio(municipio: any) {
    //Primero capturamos los datos de path y arreglo de objetos con el path de las imágenes para borrarlas del Storage
    const pathImgPrincipal = municipio.pathImagePortada.path; //path para borrar imagen portada
    const arrayPathImages = municipio.pathImages; // arreglo de Objetos de tipo PathImage
    ...
    if (!(pathImgPrincipal === '')) {
      ...
      deleteObject(refImgPrincipal)
        .then(() => {
          console.log('Se ha borrado la img Principal de: ', municipio.name);
        })
        .catch((error) => {
